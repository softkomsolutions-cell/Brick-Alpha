// Canonical Brick Alpha model, backend port of frontend/src/brickAlphaModel.js (Phase 0/4).
// This file reproduces the frontend model EXACTLY so backend and frontend outputs are
// identical for the same inputs. It does not itself read providers or persist anything.
const MS_PER_DAY = 24 * 60 * 60 * 1000;

const BRICK_ALPHA_MODEL_VERSION = 'brick-alpha-v1';
...
function retirementOutlookFor(item, today = new Date()) {
  if (item.actualRetirementDate) {
    return {
...
const THEME_ALLOCATION_TARGETS = {
  'Star Wars': 38,
  Icons: 24,
  Ideas: 15,
  Marvel: 12,
...
function themeAllocationFor(trades = []) {
...
function portfolioFitFor(item, trades = []) {
...
function investmentGradeFor(score) {
...
const BRICK_ALPHA_SIGNALS = ['High Conviction', 'Retiring Soon', 'Deep Value', 'Minifigure Alpha'];

function alphaSignalsFor(item) {
  const signals = [];
...
function alphaSignalTone(signal) {
...
function enrichBrickAlphaCollectible(item, today = new Date()) {
...
function enrichBrickAlphaTrade(trade, collectibleItems = [], allTrades = []) {
...
function availabilityStatusFor(item) {
...
function buildBrickAlphaScoreBreakdown(item) {
...
function buildPriceForecast(item, years) {
...
function buildAiCommentary(item) {
  const discount = numberOrZero(item.discountPercentage);
  const supplyNote =
    discount >= 12
...
function summarizeBrickAlphaPortfolio(trades) {
  const collectibleTrades = trades.filter((trade) => trade.assetClass === 'collectible');
